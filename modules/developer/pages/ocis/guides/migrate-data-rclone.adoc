...
Check the link:https://rclone.org/install/[rclone website on how to install] rclone.
...
You need to add a configuration for both the source cloud (Nextcloud) and the target (Infinite Scale). As both talk WebDAV, the link:https://rclone.org/webdav/[rclone manual page] is accurate to follow.
...
'''
To improve this guide, you are welcome to file an issue or even send a pull request. See the link:https://owncloud.dev/ocis/development/build-docs/[getting started guide] how easy it is to build the documentation.
