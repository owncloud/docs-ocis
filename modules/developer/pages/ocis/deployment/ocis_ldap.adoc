...
// SHORTCODE: {{< hint type=tip title=Encoding >}}
...
Open https://ocis.owncloud.test in your browser and accept the invalid certificate warning. You now can login to oCIS with the default users, which also can be found here: xref:../getting-started#login-to-ocis-web.adoc[Getting started]. You may need to wait some minutes until all services are fully ready, so make sure that you try to reload the pages from time to time.
