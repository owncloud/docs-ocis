...
A _reference_ is a logical concept. It identifies a xref:#resources.adoc[_resource_] and consists of a `<resource_id>` and a `<path>`. A `<resource_id>` consists of a `<storage_id>` and a `<node_id>`. They can be concatenated using the separators `!` and `:`:
----
...
`<storage_space>` should be a UUID to prevent references from breaking when a _user_ or xref:#storage-spaces.adoc[_storage space_] gets renamed. But it can also be derived from a migration of an oc10 instance by concatenating an instance identifier and the numeric storage id from oc10, e.g. `oc10-instance-a$1234`.
