...
- iterating over the root of the storage and treating every folder following the `<user_layout>` as a `home` _storage space_,
- iterating over the root of the storage and treating every folder following a new `<project_layout>` as a `project` _storage space_, or
- iterating over the root of the storage and treating every folder following a generic `<layout>` as a _storage space_ for a configurable space type, or
