...
** For Major and Minor versions: `8.2.0`
** For Patch releases: `8.2.1`

* The process differs slighly when creating a patch release only.
* Changes applied to a `stable-8.2` branch valid for documentation will be processed to the `docs-stable-x.y` branch automatically after merging.
====
...
. Sign-off the Releasing Process
...
* [ ] Start the Releasing Process
  * [ ] Create a `stable-8.2` branch based on current `master`
  * [ ] Inform documentation that the releasing process has started

* [ ] Start the Release Candidates Phase
  * [ ] Create a new feature branch based on `stable-8.2`
  * [ ] Move all changelog items from `unreleased` to `8.2.0-rc.1_2025-06-12`
  * [ ] Bump oCIS version in `ocis-pkg/version/version.go`
  * [ ] Bump oCIS version in `sonar-project.properties`
  * [ ] Create PR with `[full-ci][k6-test]` against `stable-8.2`
  * [ ] Get PR approved. **DO NOT MERGE YET**
  * [ ] Wait for pipeline to be green
  * [ ] Create tag: `git tag -s v8.2.0-rc.1 -m "Release 8.2.0-rc.1"`
  * [ ] Push tag: `git push origin v8.2.0-rc.1`
...
* [ ] Prepare Release
  * [ ] Create a new feature branch based on `stable-8.2`
  * [ ] Move all changelog items from `8.2.0-rc.*` to `8.2.0_2025-04-01`
  * [ ] Bump oCIS version in `ocis-pkg/version/version.go`
  * [ ] Bump oCIS version in `sonar-project.properties`
  * [ ] Create PR with `[full-ci][k6-test]` against `stable-8.2`
...
* [ ] Postprocessing and Finalization
  * [ ] Backport `stable-8.2` to the master branch
  * [ ] Create a `docs-stable-8.2` branch
    * [ ] Create orphan branch: `git checkout --orphan docs-stable-8.2`
    * [ ] Initial commit: `git commit --allow-empty -m "initial commit"`
    * [ ] Push it: `git push`
  * [ ] Adjust the `.drone.star` file to write to `docs-stable-8.2`
    * [ ] Find target_branch value in the docs section and change it to `docs-stable-8.2`
...
. Sign-off the Releasing Process
. Release the new Version
. Check Forward Porting to Master
...
* [ ] Prepare Release
  * [ ] Create a new feature branch based on `stable-8.2`
  * [ ] Move all changelog items from `unreleased` to `8.2.1_2026-07-01`
  * [ ] Bump oCIS version in `ocis-pkg/version/version.go`
  * [ ] Bump oCIS version in `sonar-project.properties`
  * [ ] Create PR with `[full-ci][k6-test]` against `stable-8.2`
  * [ ] Mark PR as **Draft** to avoid accidentially merging
...
* [ ] Release the new Version
  * [ ] Create tag: `git tag -s v8.2.1 -m "Release 8.2.1"`\
  Note the tag name scheme is important
  * [ ] Push tag: `git push origin v8.2.1`
  * [ ] Watch the PR to see pipeline succeed (can be restarted)
  * [ ] Smoke test docker image `owncloud/ocis@v8.2.1`
    * [ ] Choose any docker-compose example from ocis repository
    * [ ] Export `OCIS_DOCKER_IMAGE=owncloud/ocis`
    * [ ] Export `OCIS_DOCKER_TAG=8.2.1`
    * [ ] `docker compose up -d`
    * [ ] Confirm oCIS version in browser and start the `upload-download-awesome` test
  * [ ] Remove the **Draft** state and Merge PR\
  This is the PR from step *Prepare Release*
...
Follow the xref:services/general-info/envvars/new-release-process.adoc[Release Process for Envvars] documentation to update the required environment variable data. These changes _must_ be present in the appropriate `stable-8.2` branch and should be applied to the respective feature branch during the development cycle. This _avoids manually backporting_ to `stable-8.2` but may be required if changes go to master instead. Changes to envvars are typically found in the `docs` folder.
