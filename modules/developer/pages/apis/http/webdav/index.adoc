...
// SHORTCODE: {{< hint type=info title="RFC" >}}
_WebDAV RFCs_
...
For development purposes the examples in the developer documentation use Basic Auth. It is disabled by default and should only be enabled by setting `PROXY_ENABLE_BASIC_AUTH` in link:../../../services/proxy/configuration/#environment-variables#environment-variables[the proxy] for development or test instances.
...
// SHORTCODE: {{< hint type=caution title="Use the Depth header with caution" >}}
_Depth: infinity_
...
// SHORTCODE: {{< tabs "response create folder" >}}
