...
If there is a change in `extended_vars.yaml` which you can identify via git when running e.g. `make -C docs docs-generate`, read the <<general_info,General Info>> section first and follow the items listed below afterwards.
