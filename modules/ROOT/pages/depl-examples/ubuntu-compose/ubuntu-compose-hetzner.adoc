...
NOTE: With the minimum configuration available on Hetzner, you can start with about €4/month for a cloud driven Infinite Scale deployment. Note that prices are subject of changes and only intended for imformational purposes.
...
This guide describes an installation of Infinite Scale on the Hetzner Cloud using Ubuntu LTS as OS and docker compose. The underlying hardware can be configured according the offers from Hetzner and your requirements. 
...
If a new Infinite Scale v5 version is available, just down the compose environment and bring it back up. Containers will update automatically and you can countinue using Infinite Scale as usual.
