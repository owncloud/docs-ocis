...
This environment requires that multiple (sub)domain names are available. You can deviate from the examples according your environment. The (sub)domain must be configured in the way, that these URL's point to the server you are using which includes DNS settings, routing and NAT settings and is *not* covered here. The same ist true when using a wildcard configuration. For a wildcard setup on the DNS, three (sub)domains must be provided to configure the `.env` file described in one of the next sections.
