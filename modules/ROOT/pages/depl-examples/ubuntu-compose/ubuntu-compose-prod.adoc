...
This environment requires that multiple subdomain names are available. They must be configured in the way that these URL`s point to the server you are using. This includes DNS settings, routing and NAT settings which is not covered here.
...
Data Location::
All data is stored in default docker volumes. You need to manually reconfigure the volumes if you want to define own paths.
...
Only a view settings need to be configured in the `.env` file:
...
TIP: Checkout the https://doc.owncloud.com/[Desktop App] or https://doc.owncloud.com/[Mobile Apps] the for enhanced usage.
...
.This command will print among other data the required Container ID 
[source,bash]
----
sudo docker ps
----

.Replace the <container_id> according the container you want to monitor the log.
[source,bash]
----
...
Follow this guide if you to fully remove the instance including all data.
