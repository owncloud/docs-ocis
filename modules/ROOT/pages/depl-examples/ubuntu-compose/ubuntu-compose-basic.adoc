...
{description} The aim of this guide is to be quickly up and running and get a feeling about the capabilities of Infinite Scale. Based on this guide, you can later derive for other requirements not handled in this documentation.

NOTE: This guide will always reference to the latest stable downloadable version of Infinite Scale.

== Requirements

=== Hardware

This guide describes an installation of Infinite Scale based on Ubuntu LTS and docker compose. The underlaying hardware of the computer (server) can be anything as listed below as long it meets the OS requirements defined in the xref:software-stack[Software Stack]:
...
Note, do not get confused by the term _server_. At the moment you install software that serves tasks others can access, independent if you run a desktop or server OS, the term server is used.
...
In case you are not sure, just use a search engine for more details.
...
For the OS, we have selected *Ubuntu LTS 22.04*. On purpose, we reference to https://ubuntu.com/download/desktop[Ubuntu Desktop] as you have a GUI including a browser preinstalled. The office suite that is offered to be installed with Ubuntu Desktop is no prerequisite and can be omitted during the OS installation process if a fresh setup is planned.
...
--
1. Based on the goal setting up an Infinite Scale instance quickly, any access to the instance is in the first step *only* possible from computer Infinite Scale is installed on and in the second step from the internal network ([.lime]#green#) the server is connected to.
+
...
Note that by default and for security reasons, docker commands need to be issued with `sudo` permissions. You can change that on your own responsibility based on the description in https://docs.docker.com/engine/install/linux-postinstall/[Manage Docker as a non-root user, window=_blank].

== Prepare the Infinite Scale Installation

The preparation installing Infinite Scale is made in several steps. When editing files, `nano` is used in the examples though any editor of choice can be selected.

=== Create a Docker Compose Directory

The relevant files necessary for `docker compose` can be at any location. For the ease of handling, a subdirectory structure inside your home directory is created. To do so, issue the following command in a shell:
...
When files got extracted, list the directory with:
...
** You can omit setting an own initial password. +
...
This comand will download all necessary containers and starts up the instance according your settings. All outputs and logging is also printed to the shell. For later startups, you can add the `-d` flag which starts the container in the background and no logging is printed to the shell.

=== Prepare Your Browser

* The following step is necessary because no valid browser certificates like ones from Letsencryp are used.
...
Now, after preparations have finished, you can access your instance *from your server* (the machine you have installed Infinite Scale). To do so, open your browser and enter the instance URL:
...
NOTE: If you forgot the manual defined password set during initial setup, you need to follow one of the procedures described in the xref:admin-password[Admin Password] section.
...
[NOTE]
====
The Infinite Scale deployment will reboot automatically on a server reboot if the compose environment is not downed manually by the following commend.
...
In the image shown at xref:accessing-ocis[Accessing Infinite Scale], a modem/router has a central role providing access to the internet. For home and small business environments, this device also runs internal services like DHCP to provide IP addresses for clients on the network and DNS to resolve names to IP addresses. These services usually do not need any manual configuration except when running dedicated services in the local network. Both, the DHCP and DNS service can, depending on your environment also be on different hardware. The following procedure describes the setup necessary in a general way, use the vendors guide for configuration details.
...
----
Note that this IP address is usually provided via DHCP and is therefore not static and subject of changes. Setting up a static IP address is not covered here.
...
+
--
Add DNS entries similar as defined in xref:update-the-hosts-file[Update the hosts File] but using the IP address of your server Infinite Scale runs on, replace the example IP address with the real address from <IP address returned from above>:
...
* Check that *all* entries from above are resolvable from one client in your local network. That client shoud not be the one running Infinite Scale. Open a shell and use the following example command:
+
--
[source,bash]
----
...
== Mointor the Instance

=== Logs

I fyou have started up the instance in background mode using:
...
you can monitor the logs life by issuing the following command:
...
For the case the manual set *initial* admin password has been forgotten *before* it got changed, you can get it from the docker log. See the https://docs.docker.com/config/containers/logging/[View container logs] for more details on docker logging.
...
From the output, note the container ID that has in the printout:
