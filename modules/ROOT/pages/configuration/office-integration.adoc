...
Installing the Docker stack only requires a few steps. Note to use the relevant tag in the URL for copying or switch to the relevant tag after cloning. Select a base folder of choice for the target location:
...
+
After copying all ressources is finished, change into the directory:
+
[source,bash]
----
cd ocis_wopi
