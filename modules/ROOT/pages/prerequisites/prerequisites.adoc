...
Most easiest, when using a kernel version 5.9 or higher, extended attributes for NFS server and NFS client are part of the system. To check, run the command:
+
[source,bash]
----
uname -r
----
+
On the system providing NFS server AND on the NFS client and check the displayed version number.
