...
== Authorization and Authentication (IDP)
ownCloud Infinite Scale is dependent on Open ID Connect, therefore a connection to an identity provider (IDP) is mandatory.
Konnect is included, however it has a limited scope and is therefore only recommended for up to 500 users and if you do not have a central IDP.
