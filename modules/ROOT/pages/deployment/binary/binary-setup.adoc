...
NOTE: oCIS by default relies on Multicast DNS (mDNS), usually via `avahi-daemon`. If your system has a firewall, make sure mDNS is allowed in your active zone.
...
* The user `ocis` is just a placeholder for any user to run oCIS, the user must exist or be created first.
+
[NOTE]
====
