...
This description gives a brief overview and can be used as basic template running the ocis binary including an example how to configure systemd. It does not cover extended deployment tasks.
...
Infinite Scale can either be downloaded from ownCloud or installed via the package manager of your Linux system which will be available soon. Use the way you prefer, but keep in mind that the download version is updated more frequently than the package manager version.
...
When you have configured and started the Infinite Scale _runtime_ either xref:starting-infinite-scale[manually] or via xref:setting-up-systemd-for-infinite-scale[systemd] as described below, you can access it via a browser using `localhost:{ocis_port}` or whatever URL and port you have defined.
