...
`$HOME/.ocis/config/`
+
NOTE: You can define a nonstandard configuration file location on startup using the environment variable `OCIS_CONFIG_FILE`
+
NOTE: When using a system user for the runtimes which has no login and therefore no home directory, you must specify the configuration file location, see the xref:deployment/manual/manual-setup.adoc#setting-up-systemd-for-ocis[Setting up systemd for oCIS] for more details.
...
[width="80%",cols="30%,30%,45%,25%,40%",options="header"]
|===
| Username
| Password
| Email
