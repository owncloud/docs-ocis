...
[source,plaintext]
----
2022/04/04 13:57:32 dialing:dial tcp 127.0.0.1:9250: connect: connection refused
----

This info contains a:

* Timestamp
* The statement that it tries to call the following address
* The connection type and address
* The reason it failed

The term _dial/connect_ in the error message above comes from the Dial function of the _net_ package in Golang which connects to a server address on a named network.
...
If you have your own certificates already in place, make oCIS use them by adding the following environment variables to the command, replace the certificates path and file names according your needs:
