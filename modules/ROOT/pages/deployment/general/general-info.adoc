...
To prepare Infinite Scale to have a base for further configuration and for recurring starts, you have to initalize it first. To do so, run:
...
You can add command line parameters. To see which one are available type:
...
Command line parameters are benificial if you e.g. want to handover all necessary parameters without getting to any questionaire or when you want to define the admin password by yourself not getting a random one.
