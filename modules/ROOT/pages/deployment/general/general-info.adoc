...
In rare cirumstances, it can happen that it is necessary to initiate reindexing manually for a given space or user. Though the search service handles exeption cases automatically, it can happen that the search service was not able to complete indexing due to a dirty shut down of the service or because of a bug. The following command should *only* be run on explicit request and supervision of ownCloud support. Note that behaviour and options may change without special notice. 
...
OPTIONS:
   --space value, -s value  the id of the space to travers and index the files of
