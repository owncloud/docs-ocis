...
:description: The Infinite Scale ocm service provides federated sharing functionality based on the sciencemesh and ocm HTTP API's.
...
The list of trusted instances is managed via a configuration file. The currently only supported backend is `json`, which stores the list of trusted instances in a json file on disk.
...
Note that there is no limit in adding multiple trusted providers.
...
The data backend for the workflow, not the data itself, is configurable. The currently only supported backend is `json` which stores the data in a json file on disk.
