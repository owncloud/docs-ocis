...
When users login, they automatically get a role assigned. The automatic role assignment can be
...
When set to `default`, all users that do not have a role assigned at the time for the first login will
