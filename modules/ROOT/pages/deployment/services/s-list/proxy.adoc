...
-   Use the environment variable xref:deployment/services/env-vars-special-scope.adoc#global-environment-variables[OCIS_URL] to define how ocis can be accessed, *mandatory* use `https` as protocol for the URL.
...
-   `PROXY_TLS` *can* be set to `false` if a reverse proxy is used and the OCIS_URL is terminated at the reverse proxy. When setting to `false`, the communication between the reverse proxy and Infinite Scale is not secured. If set to `true`, you must provide certificates.
