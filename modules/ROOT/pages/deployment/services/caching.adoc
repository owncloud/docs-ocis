...
* Persistent caches can be identified via environment variables ending with `xxx_STORE` when using local and `OCIS_PERSISTENT_STORE` when using global ones. Note that no other trailing name components are present when using local ones.
