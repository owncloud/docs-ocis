...
[IMPORTANT]
====
When using docker-compose, there are common networking pitfalls. Though these pitfalls are not restricted to caching and persistence and without completeness, here are some examples:

- When connecting to an internal network, ports of external services should not be exposed for security reasons.
- Check if the store that is configured both in Infinite Scale and docker-compose is reachable from the ocis like:
+
[source,bash]
----
