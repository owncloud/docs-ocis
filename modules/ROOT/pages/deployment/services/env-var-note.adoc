...
Environment variables starting with `OCIS_` are variables that have a global scope. This means, their setting is valid for all services, if not explicitly overwritten by a correspondent service varibale.
