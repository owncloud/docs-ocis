...
It's important to note, that the feature is at the moment only capable providing static (js, mjs, e.g.) web applications and does not support injection of dynamic web applications (custom dynamic backends).
...
Additionally, the administrator can provide custom applications by storing them in the path defined the environment variable `WEB_ASSET_APPS_PATH`.
...
The final list of available applications is composed of the build in and the custom applications provided by the administrator via `WEB_ASSET_APPS_PATH`.

For example, if Infinite Scale would contain a build in extension named `image-viewer-dfx` and the administrator provides a custom application named `image-viewer-obj` via the `WEB_ASSET_APPS_PATH` directory, the user will be able to access both
applications from the WebUI.

=== Application Structure

* Applications always have to follow a strict structure, which are:
...
Please note that Infinite Scale, respectively the web service needs a restart to load new applications or changes to the `apps.yaml` file.
