...
`ACCOUNTS_DEMO_USERS_AND_GROUPS=false ./bin/ocis server`. In this case, only the user Admin is created plus the system users for IDP and Reva.

After you have added an additional user with admin priviledges, either delete the demo users in ownCloud Web or use the command `ocis accounts remove <ID>`.
