...
----
+
To terminate the `ocis proxy` service type the following command, where `sudo` can be necessary depending on the permissions the logged on user has. Replace the PID according the output from above:
+
[source,bash]
----
sudo kill -15 221706
