...
Note that the `values.yaml` file provided by ownCloud uses generic configuration. You can customise this provided configuration with your own values, for example for different setups or sizings. This should be done by using your own `values.yaml` file at a different location which will _overwrite_ or _add_ content to the provided one. While not mandatory, the identical file name follows the convention of Helm. When it comes to security sensitive data like secrets, such data is usually _not_ added in the overwrite-values file for security reasons. In such a case, you apply secrets via command from a secrets file.

=== Supported Infinite Scale Versions

////

when a new Helm chart version is published on the github repo, we need to adapt following settings:

* in `antora.yml`: check, set and name the attributes for the tab attributes accordingly
  the `tag_x` value needs to match the tag set in the referenced git repo!
  the `tag_x_tab_text` values will be used as printout text which can be different from `tag_x`.
* in `modules/ROOT/examples/deployment/container/orchestration` check/adapt the contents of the files
* only change the secrets attribute if
  - the orchestration.adoc page gets relocated or
  - the section header changes (like for renaming)

Note that you do not need to manipulate any other files as they are included from source (here/git) automagically
...
// note that the sources for the include are are on github.
...
// note that the sources for values and description are are on github.
...
* Create your own local `values.yaml` file which overwrites parts of the provided one with the following contents:
+
[tabs]
====
{tab_1_tab_text}::
...
Infinite Scale requires some secrets to work. It was decided not to create them automagically, because Helm does not support _one-off_ generation of secrets out of the box. Certificates are also required which should have an expiry and therefore need a certificate rotation from time to time, which is also not supported by Helm.

These reasons add up and as a result, the responsibility for these secrets and their lifecycle is handed over to the operator who uses the Helm Charts. Any information necessary to use the security relevant data is provided by ownCloud via examples.

// edit the yaml data shown at example$deployment/container/orchestration/

:t_text: Secrets file containing all relevant secrets configurations.

* The following example shows what secrets need to look like and how they can be generated. The example assumes that the `secretRefs` are not changed . Each secret data entry holds a description on how to generate it or find the right value.
+
[tabs]
====
{tab_1_tab_text}::
...
| xref:deployment/container/orchestration/secrets-tab-1.adoc[secrets.yaml,window=_blank]
| {t_text}
|===
--
ifdef::use_tab_2[]
...
| xref:deployment/container/orchestration/secrets-tab-2.adoc[secrets.yaml,window=_blank]
| {t_text}
|===
--
endif::[]
...
| xref:deployment/container/orchestration/secrets-tab-3.adoc[secrets.yaml,window=_blank]
| {t_text}
|===
--
endif::[]
...
. To apply secrets by command, save the content as `secrets.yaml` and use the following command, add a path to the secrets file if necessary:
+
[source,bash]
----
...
If you're using the builtin user management with setting `features.externalUserManagement.enabled` set to `false`, you need to set these secrets. Certificates are also required which should have an expiry and therefore need a certificate rotation from time to time, which is also not supported by Helm.

These reasons add up and as a result, the responsibility for these user management secrets and their lifecycle is handed over to the operator who uses the Helm Charts. Any information necessary to use the security relevant data is provided by ownCloud via examples.
...
. To apply secrets by command, save the content as `user-secrets.yaml` and use the following command, add a path to the secrets file if necessary:
+
[source,bash]
----
...
If you're using external user management with setting `features.externalUserManagement.enabled` set to `true`, you need to set these secrets. Certificates are also required which should have an expiry and therefore need a certificate rotation from time to time, which is also not supported by Helm.
...
* Install the changes provided via your own `values.yaml` file:
+
[source,bash]
----
...
. Add the minikube IP address to your `/etc/hosts` file or use the Ingress DNS addon. An example line for the `/etc/hosts` file could look like this:
+
[source,plaintext]
----
