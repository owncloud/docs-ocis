...
Infinite Scale requires some mandatory Secrets and ConfigMaps to work. They are created _one-off_ if you don't explicitly provide them. If you're using the builtin user management (not recommended), among the auto generated Secrets there are also some certificates which expire and need to be renewed by you.
Please not that these Secrets and ConfigMaps need to be part of your backup since you need to provide them manually during a disaster recovery procedure.
...
* If you want to manage ConfigMaps on your own, you can look at the following example which shows what mandatory ConfigMaps look like and how they can be generated. The example assumes that the `configRefs` are not changed. Each ConfigMaps data entry holds a description of how to generate it or find the right value.
...
If you're using the built-in user management by setting `features.externalUserManagement.enabled` to `false` (which is the default), you'll need additional Secrets. These are also autogenerated for you if you don't provide them. Among these Secrets are certificates that expire after 365 days and therefore need a certificate rotation from time to time. Rotation can be achieved by deleting the Secrets "ldap-ca" and "ldap-cert" and restarting all oCIS Deployments (eg. `kubectl rollout restart deploy`)
