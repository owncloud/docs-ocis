...
----
+
The IP address to be set, which should normally be the one of the `en0` interface, can be fetched from the `ifconfig` command. Note that the interface must be reachable from within Kubernetes and `127.0.0.1` is no option for this case. This is due to the fact that the OpenID process tries to lookup the hostname of the cluster via the  `/etc/hosts` file which is described in the next step and `127.0.0.1` within a pod will not resolve to the host serving Kubernetes.

. Add the IP address specified by the above's command to your `/etc/hosts` file. An example could look like this:
+
[source,plaintext]
----
