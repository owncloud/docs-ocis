...
----
+
Follow step `4) (optional) Configure in-cluster DNS server to resolve local DNS names inside cluster` from https://minikube.sigs.k8s.io/docs/handbook/addons/ingress-dns/#installation. Please note that this is not optional for a oCIS installation.
...
. Configure hosts
+
On MacOS you need to add additional configurations to use the ingress by adding the domain names to `/etc/hosts`. Since you are using `minikube tunnel`, those entries need to point to `127.0.0.1` because it's listening on the localhost interface.
+
[source,bash]
----
...
----
+
On Linux you need to add additional configurations to use the ingress by adding the domain names to `/etc/hosts`. Those entries need to point to the Minikube interface IP which you can get by running `minikube ip`.
+
[source,bash]
----
...
* When installing oCIS in Minikube on MacOS you need to set the `hostAlias` option:
+
[source,yaml]
----
hostAlias:
