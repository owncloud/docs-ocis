...
// this is an additional anchor for yaml files that get included and reference to this section which was renamed
...
* The following example shows what generic secrets need to look like and how they can be generated. The example assumes that the `secretRefs` are not changed . Each secret data entry holds a description of how to generate it or find the right value.
+
[tabs]
====
{helm_tab_1_tab_text}::
...
* The following example shows what user management secrets need to look like and how they can be generated. The example assumes that the `secretRefs` are not changed . Each secret data entry holds a description of how to generate it or find the right value.
+
[tabs]
====
{helm_tab_1_tab_text}::
...
. minikube on MacOS requires a tunnel to be started from minikube to get access to the ingress of the cluster:
+
[source,bash]
----
...
----
+
The address to be set here (which should normally be the one of `en0`) can be fetched from `ifconfig`. Please note that the interface must be reachable from within Kubernetes. So `127.0.0.1` is no option here. This is due to the fact that the OpenID process tries to lookup the hostname of the cluster via the hosts `/etc/hosts` file (see next step), but `127.0.0.1` within a pod will not resolve to the host serving Kubernetes.

. Add the IP address you specified in above's command to your `/etc/hosts` file. An example line for the `/etc/hosts` file could look like this:
+
[source,plaintext]
----
...
----
+
Warning: This way you expose the system on your local machine's port to the local intranet. So everybody in your home network can access the system.
