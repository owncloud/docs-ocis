...
Default values containing curly braces::
+
[TIP]
====
When looking into the table showing the environment variables of an extension, the _Default Value_ cell can sometimes contain values with curly braces like `{{.Id.OpaqueId}}`. These are valid default values.
====
