...
* Internally, a space is identified by a unique `storage space id` and shows up to users by a given name.
* A space has no ownership, though one or more users can be assigned to manage this space.
...
* A space makes it much more streamlined to separate data that can be shared or should not be shared. As an example, though you can share content in your personal space, it is much more secure to have a space dedicated for sharing where only you are the manager of, separating data from your personal space that is for your eyes only.
