...
A storage space is a logical concept. It organizes a set of resources in a hierarchical tree. It is identified by a `storage space id`, but no ownership is needed though one or more users are assigned to manage this space. Spaces can have users or groups, a quota, permissions, an image and a description etc. assigned. They may serve different purposes and have different workflows enabled like antivirus scanning. Examples for spaces would be every user’s personal storage space, project or group or class storage spaces including shares and reshares.
