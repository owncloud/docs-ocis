...
** You can also use a wildcard operator for the subdomains in the DNS configuration. In case where you already use subdomains for other purposes, we recommend using a subdomain as entry point and add the wildcard operator there to allow different IP configurations.
...
This environment requires that multiple (sub)domain names are available. You have to deviate from the examples according your environment. The (sub)domains must be configured to point to the server you configured. The same ist true when using a wildcard configuration. For a wildcard setup on the DNS, three (sub)domains must be provided to configure the `.env` file described in one of the next sections.
...
To be prepared accessing the configured server, we recommend using key based authentication for ssh instead using user and password. This is not only beneficial for security reasons but also because you can define the public key to be installed  during the initial server configuration.
...
If you not already have an account on https://www.hetzner.com[Hetzner], register for free and log in.
...
--
* *Server Location* (Chose one from the offered)
* *Image* (we use for this deployment Ubuntu)
* *Type* (select any server type that matches your requirements)
* *Networking* (we recommend using additionally IPv4)
...
* *Firewall* (add a rule for at minimum port 22 and 443, can be added later on too)
* ... more items
...
Whenever you had a DNS issue at this stage, you will face follow up issues on a consecutive compose start because the certificae volume now holds invalid data. Therefore the cert volume needs to be deleted:
...
docker compose down
----
Note, not use the `-v` option as it will delete ALL volumes. Also see the next section.
