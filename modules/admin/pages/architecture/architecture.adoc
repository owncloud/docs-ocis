...
To create a truly federated storage architecture, Infinite Scale breaks down the ownCloud Classic 10 user-specific namespace, which is assembled on the server side, and makes the individual parts accessible to clients as storage spaces and storage space registries.
