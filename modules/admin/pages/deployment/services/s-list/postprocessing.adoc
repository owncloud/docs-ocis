...
:page-aliases: ROOT:deployment/services/s-list/postprocessing.adoc
...
The postprocessing service provides a CLI which can be used to check its health status and to resume/restart uploads. See the following printout for a shortened list:

[source,bash]
----
ocis postprocessing --help

USAGE:
   ocis postprocessing [command options]

COMMANDS:
   resume, restart  resume postprocessing for an uploadID
   health           Check health status
----

=== Health

The `{service_name}` service provides a `health` command. For details see the xref:maintenance/commands/commands.adoc[Maintenance Commands] description.

=== Resume Post-Processing

If post-processing fails in one step due to an unforeseen error, current uploads will not be resumed/restarted automatically. A system administrator can instead run CLI commands to resume/restart the failed upload manually.

The `{service_name}` service provides a `restart` and `resume` command which share common options:

[source,plaintext]
----
OPTIONS:
   --upload-id value, -u value  the uploadid to resume. Ignored if unset.
   --step value, -s value       resume all uploads in the given postprocessing step. Ignored if upload-id is set. (default: "finished")
----
...
The resume/restart command is, at a minimum, a two-step process that requires commands from the `storage-users` service.
