...
:page-aliases: ROOT: security/security.adoc
...
Compared to the apps/extensions in ownCloud Classic 10, the Infinite Scale extensions/services are more secure because they are developed and deployed as standalone services. They communicate via APIs with the core services and therefore can only use specific interfaces instead of having direct access to storages and data. A single extension which behaves incorrectly cannot corrupt the whole system.
